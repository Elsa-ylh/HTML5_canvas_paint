...
describe('SidebarComponent', () => {
    ...
    */
    ...
    it('should pick selection', () => {
        const switchToolSpy = spyOn<any>(toolServiceStub, 'switchTool').and.stub();
        component.pickSelection(1);
        expect(drawingStub.cursorUsed).toEqual(cursorName.default);
        expect(switchToolSpy).toHaveBeenCalled();
        expect(toolServiceStub.currentTool.subToolSelect).toEqual(1);
    });
    it('should set all checked to false', () => {
        ...
        expect(component.selectionChecked).toEqual(false);
    ...
    */
    ...
    it(' should call resetCheckButton set isSelectionChecked and isSelectionRectangleChecked to true should call pickSelection', () => {
        toolServiceStub.currentToolName = ToolUsed.Pencil;
        const event = new KeyboardEvent('window:keydown.r', {});
        const spyReset = spyOn(component, 'resetCheckedButton').and.callThrough();
        const spyPickSelection = spyOn(component, 'pickSelection').and.callThrough();
        window.dispatchEvent(event);
        component.changeSelectionRectangleMode(event);
        expect(spyReset).toHaveBeenCalled();
        expect(component.selectionChecked).toEqual(true);
        expect(component.selectionRectangleChecked).toEqual(true);
        expect(spyPickSelection).toHaveBeenCalled();
    });

    it(' should call resetCheckButton set isSelectionChecked and isSelectionEllipseChecked to true should call pickSelection', () => {
        toolServiceStub.currentToolName = ToolUsed.Pencil;
    ...
    it('should call prevent default and selectAll', () => {
        toolServiceStub.currentToolName = ToolUsed.Selection;
        const event = new KeyboardEvent('window:keydown.control.a', {});
        const spyPreventDefault = spyOn(event, 'preventDefault').and.callThrough();
        const spySelectAll = spyOn(selectionStub, 'selectAll').and.stub();
        window.dispatchEvent(event);
        component.selectAllCanvas(event);
        expect(spyPreventDefault).toHaveBeenCalled();
        expect(spySelectAll).toHaveBeenCalled();
    });
});
